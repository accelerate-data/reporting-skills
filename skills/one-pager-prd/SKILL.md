...
For simple features needing quick approval → Use [references/template.md](references/template.md) one-pager format (1 page, bullets). For complex features/products requiring detailed requirements → Use [references/template.md](references/template.md) full PRD format (1-2 pages). For writing guidance and structure → Study [references/methodology.md](references/methodology.md) for problem framing, metric definition, scope techniques.
...
Self-assess using [references/rubric-one-pager-prd.json](references/rubric-one-pager-prd.json). Check: problem is specific and user-focused, solution is clear without being overly detailed, metrics are measurable and have targets, scope is realistic and boundaries clear, constraints acknowledged, open questions identified. Minimum standard: Average score ≥ 3.5.
...
**Resources:**
- `references/template.md` - One-pager and PRD templates with section guidance
- `references/methodology.md` - Problem framing techniques, metric trees, scope prioritization, writing clarity
- `references/rubric-one-pager-prd.json` - Quality criteria
